...
from powerdns_admin.models import (
    User,
    Account,
    AccountUser,
    Domain,
...
from powerdns_admin.lib import utils
import powerdns_admin.oauth as oauth
from powerdns_admin.decorators import (
    admin_role_required,
...
from powerdns_admin.customboxes import customBoxes
from sqlalchemy import not_
from powerdns_admin.app import csrf, login_manager
...
@views_blueprint.before_app_first_request
...
def before_request():
    ...
    if (
        ...
        and current_user.role.name not in ["Administrator", "Operator"]
    ):
...
# START VIEWS
@views_blueprint.errorhandler(400)
def http_bad_request(e):
    return redirect(url_for("views_blueprint.error", code=400))


@views_blueprint.errorhandler(401)
def http_unauthorized(e):
    return redirect(url_for("views_blueprint.error", code=401))


@views_blueprint.errorhandler(404)
@views_blueprint.errorhandler(405)
def _handle_api_error(ex):
    if request.path.startswith("/api/"):
        return json.dumps({"msg": "NotFound"}), 404
    else:
        return redirect(url_for("views_blueprint.error", code=404))


@views_blueprint.errorhandler(500)
def http_page_not_found(e):
    return redirect(url_for("views_blueprint.error", code=500))


@views_blueprint.route("/error/<path:code>")
...
@views_blueprint.route("/swagger", methods=["GET"])
def swagger_spec():
    try:
    ...
    if Setting().get("signup_enabled"):
        if request.method == "GET":
            ...
            if password != rpassword:
                return render_template(
                    "register.html", error="Password confirmation does not match"
                )
...
@views_blueprint.route("/google/login")
def google_login():
    if not Setting().get("google_oauth_enabled") or google is None:
        ...
        redirect_uri = url_for("views_blueprint.google_authorized", _external=True)
        return google.authorize_redirect(redirect_uri)


@views_blueprint.route("/github/login")
def github_login():
    if not Setting().get("github_oauth_enabled") or github is None:
        ...
        redirect_uri = url_for("views_blueprint.github_authorized", _external=True)
        return github.authorize_redirect(redirect_uri)


@views_blueprint.route("/oidc/login")
def oidc_login():
    if not Setting().get("oidc_oauth_enabled") or oidc is None:
        ...
        redirect_uri = url_for("views_blueprint.oidc_authorized", _external=True)
        return oidc.authorize_redirect(redirect_uri)


@views_blueprint.route("/saml/login")
def saml_login():
    if not app.config.get("SAML_ENABLED"):
        return abort(400)
    req = utils.prepare_flask_request(request)
    auth = utils.init_saml_auth(req, app)
    redirect_url = OneLogin_Saml2_Utils.get_self_url(req) + url_for(
        "views_blueprint.saml_authorized"
    )
    return redirect(auth.login(return_to=redirect_url))


@views_blueprint.route("/saml/metadata")
def saml_metadata():
    if not app.config.get("SAML_ENABLED"):
        return abort(400)
    req = utils.prepare_flask_request(request)
...
@views_blueprint.route("/saml/authorized", methods=["GET", "POST"])
@csrf.exempt
def saml_authorized():
    errors = []
    if not app.config.get("SAML_ENABLED"):
    ...
    if len(errors) == 0:
        ...
        if app.config.get("SAML_ATTRIBUTE_USERNAME", False):
            username = session["samlUserdata"][app.config["SAML_ATTRIBUTE_USERNAME"]][
                0
            ].lower()
        else:
        ...
        if not user:
            # create user
            user = User(
                username=username, plain_text_password=None, email=session["samlNameId"]
            )
            user.create_local_user()
        session["user_id"] = user.id
        email_attribute_name = app.config.get("SAML_ATTRIBUTE_EMAIL", "email")
        givenname_attribute_name = app.config.get(
        ...
        if admin_attribute_name:
            ...
            for account_name in session["samlUserdata"].get(account_attribute_name, []):
                ...
                if not account:
                    account = Account(
                        name=clean_name.lower(), description="", contact="", mail=""
                    )
                    account.create_account()
                    history = History(
                        msg="Account {0} created".format(account.name),
            ...
            for account in user_accounts - saml_accounts:
                ...
                history = History(
                    msg="Removing {0} from account {1}".format(
...
def login():
    ...
    if g.user is not None and current_user.is_authenticated:
        return redirect(url_for("views_blueprint.dashboard"))
    ...
    try:
        ...
        if auth == False:
            return render_template(
                "login.html", saml_enabled=SAML_ENABLED, error="Invalid credentials"
            )
    except Exception as e:
    ...
    if user.otp_secret:
        if otp_token and otp_token.isdigit():
            ...
            return render_template(
                "login.html", saml_enabled=SAML_ENABLED, error="Token required"
            )

    login_user(user, remember=remember_me)
    return redirect(session.get("next", url_for("views_blueprint.index")))
...
def logout():
    if (
        app.config.get("SAML_ENABLED")
        and "samlSessionIndex" in session
        and app.config.get("SAML_LOGOUT")
    ):
        req = utils.prepare_flask_request(request)
        auth = utils.init_saml_auth(req, app)
...
@views_blueprint.route("/saml/sls")
def saml_logout():
    req = utils.prepare_flask_request(request)
    auth = utils.init_saml_auth(req, app)
...
@views_blueprint.route("/dashboard_domains_custom/<path:boxId>", methods=["GET"])
@login_required
...
def dashboard():
    if (
        not Setting().get("pdns_api_url")
        or not Setting().get("pdns_api_key")
        or not Setting().get("pdns_version")
    ):
        return redirect(url_for("views_blueprint.admin_setting_pdns"))
    ...
    if statistics:
        uptime = list([uptime for uptime in statistics if uptime["name"] == "uptime"])[
            0
        ]["value"]
    else:
        uptime = 0
...
@views_blueprint.route("/dashboard-domains", methods=["GET"])
@login_required
...
@views_blueprint.route("/dashboard-domains-updater", methods=["GET", "POST"])
@login_required
...
@views_blueprint.route("/domain/<path:domain_name>", methods=["GET"])
@login_required
@can_access_domain
...
@views_blueprint.route("/admin/domain/add", methods=["GET", "POST"])
@login_required
@can_create_domain
def domain_add():
    ...
    if request.method == "POST":
        try:
            ...
            if " " in domain_name or not domain_name or not domain_type:
                return (
                    render_template("errors/400.html", msg="Please correct your input"),
                    400,
                )
            ...
            if result["status"] == "ok":
                history = History(
                    msg="Add domain {0}".format(domain_name),
                    detail=str(
                        {
                            "domain_type": domain_type,
                            "domain_master_ips": domain_master_ips,
                ...
                if domain_template != "0":
                    template = DomainTemplate.query.filter(
                        DomainTemplate.id == domain_template
                    ).first()
                    template_records = DomainTemplateRecord.query.filter(
                        DomainTemplateRecord.template_id == domain_template
                    ).all()
                    record_data = []
                    ...
                    if result["status"] == "ok":
                        history = History(
                            msg="Applying template {0} to {1}, created records successfully.".format(
                                template.name, domain_name
                            ),
                            detail=str(result),
                            created_by=current_user.username,
                        )
                        ...
                        history = History(
                            msg="Applying template {0} to {1}, FAILED to created records.".format(
                                template.name, domain_name
                            ),
                            detail=str(result),
                            created_by=current_user.username,
        ...
        return render_template(
            "domain_add.html", templates=templates, accounts=accounts
        )


@views_blueprint.route("/admin/domain/<path:domain_name>/delete", methods=["POST"])
@login_required
@operator_role_required
def domain_delete(domain_name):
    ...
    history = History(
        msg="Delete domain {0}".format(domain_name), created_by=current_user.username
    )
    history.add()
...
@views_blueprint.route(
    "/admin/domain/<path:domain_name>/manage", methods=["GET", "POST"]
)
@login_required
@operator_role_required
def domain_management(domain_name):
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("views_blueprint.domain_management", domain_name=domain_name)
        )


@views_blueprint.route(
    "/admin/domain/<path:domain_name>/change_soa_setting", methods=["POST"]
)
@login_required
@operator_role_required
...
@views_blueprint.route(
    "/admin/domain/<path:domain_name>/change_account", methods=["POST"]
)
@login_required
@operator_role_required
def domain_change_account(domain_name):
    ...
    if status["status"]:
        return redirect(
            url_for("views_blueprint.domain_management", domain_name=domain.name)
        )
    else:
        return redirect(url_for("views_blueprint.error", code=500))
...
@views_blueprint.route(
    "/domain/<path:domain_name>/apply", methods=["POST"], strict_slashes=False
)
@login_required
@can_access_domain
def record_apply(domain_name):
    ...
    try:
        ...
        if domain:
            if int(submitted_serial) != domain.serial:
                return make_response(
                    jsonify(
                        {
                            "status": "error",
                            "msg": "The zone has been changed by another session or user. Please refresh this web page to load updated records.",
                        }
                    ),
                    500,
                )
            ...
            return make_response(
                jsonify(
                    {
                        "status": "error",
                        "msg": "Domain name {0} does not exist".format(domain_name),
                    }
                ),
                404,
            )
        ...
        logging.debug(traceback.format_exc())
        return make_response(
            jsonify({"status": "error", "msg": "Error when applying new changes"}), 500
        )


@views_blueprint.route(
    "/domain/<path:domain_name>/update", methods=["POST"], strict_slashes=False
)
@login_required
@can_access_domain
def record_update(domain_name):
    ...
    try:
        ...
        if result["status"] == "ok":
            ...
            return make_response(
                jsonify({"status": "error", "msg": result["msg"]}), 500
            )
    except Exception as e:
...
@views_blueprint.route("/domain/<path:domain_name>/info", methods=["GET"])
@login_required
@can_access_domain
...
@views_blueprint.route("/domain/<path:domain_name>/dnssec", methods=["GET"])
@login_required
@can_access_domain
...
@views_blueprint.route("/domain/<path:domain_name>/dnssec/enable", methods=["POST"])
@login_required
@can_access_domain
@can_configure_dnssec
...
@views_blueprint.route("/domain/<path:domain_name>/dnssec/disable", methods=["POST"])
@login_required
@can_access_domain
@can_configure_dnssec
...
@views_blueprint.route(
    "/domain/<path:domain_name>/managesetting", methods=["GET", "POST"]
)
@login_required
@operator_role_required
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if jdata["action"] == "set_setting":
                ...
                domain = Domain.query.filter(Domain.name == domain_name).first()
                setting = (
                    DomainSetting.query.filter(DomainSetting.domain == domain)
                ...
                if setting:
                    if setting.set(new_value):
                        history = History(
                            msg="Setting {0} changed value to {1} for {2}".format(
                        ...
                        history.add()
                        return make_response(
                            jsonify({"status": "ok", "msg": "Setting updated."})
                        )
                    else:
                        return make_response(
                            jsonify(
                    ...
                    if domain.add_setting(new_setting, new_value):
                        history = History(
                            msg="New setting {0} with value {1} for {2} has been created".format(
                                new_setting, new_value, domain.name
                            ),
                            created_by=current_user.username,
                        )
                ...
                return make_response(
                    jsonify({"status": "error", "msg": "Action not supported."}), 400
                )
        except Exception as e:
...
@views_blueprint.route("/templates", methods=["GET", "POST"])
@views_blueprint.route("/templates/list", methods=["GET", "POST"])
@login_required
@operator_role_required
def templates():
    templates = DomainTemplate.query.all()
...
@views_blueprint.route("/template/create", methods=["GET", "POST"])
@login_required
@operator_role_required
def create_template():
    if request.method == "GET":
    ...
    if request.method == "POST":
        try:
            ...
            if " " in name or not name or not type:
                ...
                return redirect(url_for("views_blueprint.create_template"))

            if DomainTemplate.query.filter(DomainTemplate.name == name).first():
                flash(
                    "A template with the name {0} already exists!".format(name), "error"
                )
                return redirect(url_for("views_blueprint.create_template"))
            ...
            if result["status"] == "ok":
                history = History(
                    msg="Add domain template {0}".format(name),
                    detail=str({"name": name, "description": description}),
                    created_by=current_user.username,
                )
                history.add()
                return redirect(url_for("views_blueprint.templates"))
            else:
...
@views_blueprint.route("/template/createfromzone", methods=["POST"])
@login_required
@operator_role_required
def create_template_from_zone():
    try:
        ...
        if " " in name or not name or not type:
            return make_response(
                jsonify({"status": "error", "msg": "Please correct template name"}), 500
            )
        ...
        if DomainTemplate.query.filter(DomainTemplate.name == name).first():
            return make_response(
                jsonify(
                    {
                        "status": "error",
                        "msg": "A template with the name {0} already exists!".format(
                            name
                        ),
                    }
                ),
        ...
        if result["status"] == "ok":
            ...
            if domain:
                ...
                if StrictVersion(Setting().get("pdns_version")) >= StrictVersion(
                    "4.0.0"
                ):
                    for jr in jrecords:
                        if jr["type"] in Setting().get_records_allow_to_edit():
                            name = (
                                "@"
                                if jr["name"] == domain_name
                                else re.sub("\.{}$".format(domain_name), "", jr["name"])
                            )
                            for subrecord in jr["records"]:
                                record = DomainTemplateRecord(
                                    name=name,
                                    type=jr["type"],
                                    status=True if subrecord["disabled"] else False,
                                    ttl=jr["ttl"],
                                    data=subrecord["content"],
                                )
                                records.append(record)
            ...
            if result_records["status"] == "ok":
                return make_response(
                    jsonify({"status": "ok", "msg": result["msg"]}), 200
                )
            else:
                t.delete_template()
                return make_response(
                    jsonify({"status": "error", "msg": result_records["msg"]}), 500
                )
...
@views_blueprint.route("/template/<path:template>/edit", methods=["GET"])
@login_required
@operator_role_required
...
@views_blueprint.route(
    "/template/<path:template>/apply", methods=["POST"], strict_slashes=False
)
@login_required
def apply_records(template):
    try:
        ...
        for j in jdata["records"]:
            ...
            dtr = DomainTemplateRecord(
                name=name, type=type, data=data, status=disabled, ttl=ttl
            )
            records.append(dtr)
        ...
        if result["status"] == "ok":
            jdata.pop("_csrf_token", None)  # don't store csrf token in the history.
            history = History(
                msg="Apply domain template record changes to domain template {0}".format(
                    template
                ),
                detail=str(json.dumps(jdata)),
                created_by=current_user.username,
        ...
        logging.error(
            "Cannot apply record changes to the template. Error: {0}".format(e)
        )
        logging.debug(traceback.format_exc())
        return make_response(
            jsonify({"status": "error", "msg": "Error when applying new changes"}), 500
...
@views_blueprint.route("/template/<path:template>/delete", methods=["POST"])
@login_required
@operator_role_required
...
@views_blueprint.route("/admin/pdns", methods=["GET"])
@login_required
@operator_role_required
def admin_pdns():
    if (
...
@views_blueprint.route("/admin/user/edit/<user_username>", methods=["GET", "POST"])
@views_blueprint.route("/admin/user/edit", methods=["GET", "POST"])
@login_required
@operator_role_required
def admin_edituser(user_username=None):
    ...
    if request.method == "GET":
        ...
        if create:
            if fdata["password"] == "":
                return render_template(
                    "admin_edituser.html", user=user, create=create, blank_password=True
                )

            result = user.create_local_user()
            history = History(
                msg="Created user {0}".format(user.username),
                created_by=current_user.username,
            )
            ...
            history = History(
                msg="Updated user {0}".format(user.username),
                created_by=current_user.username,
            )

        if result["status"]:
            history.add()
            return redirect(url_for("views_blueprint.admin_manageuser"))

        return render_template(
            "admin_edituser.html", user=user, create=create, error=result["msg"]
        )


@views_blueprint.route("/admin/manageuser", methods=["GET", "POST"])
@login_required
@operator_role_required
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if jdata["action"] == "user_otp_disable":
                ...
                if result:
                    history = History(
                        msg="Two factor authentication disabled for user {0}".format(
                            data
                        ),
                        created_by=current_user.username,
                    )
                    ...
                    return make_response(
                        jsonify(
                            {
                                "status": "ok",
                                "msg": "Two factor authentication has been disabled for user.",
                            }
                        ),
                        200,
                    )
                    ...
                    return make_response(
                        jsonify(
                            {
                                "status": "error",
                                "msg": "Cannot disable two factor authentication for user.",
                            }
                        ),
                        500,
                    )
            ...
            if jdata["action"] == "delete_user":
                ...
                if user.username == current_user.username:
                    return make_response(
                        jsonify(
                            {"status": "error", "msg": "You cannot delete yourself."}
                        ),
                        400,
                    )
                result = user.delete()
                if result:
                    history = History(
                        msg="Delete username {0}".format(data),
                        created_by=current_user.username,
                    )
                    history.add()
                    return make_response(
                        jsonify({"status": "ok", "msg": "User has been removed."}), 200
                    )
                else:
                    return make_response(
                        jsonify({"status": "error", "msg": "Cannot remove user."}), 500
                    )
                ...
                if result:
                    ...
                    history.add()
                    return make_response(
                        jsonify({"status": "ok", "msg": "Revoked user privileges."}),
                        200,
                    )
                else:
                    return make_response(
                        jsonify(
                            {"status": "error", "msg": "Cannot revoke user privilege."}
                        ),
                        500,
                    )
                ...
                if not user:
                    return make_response(
                        jsonify({"status": "error", "msg": "User does not exist."}), 404
                    )
                ...
                if (
                    ...
                    return make_response(
                        jsonify(
                            {
                                "status": "error",
                                "msg": "You do not have permission to change Administrator users role.",
                            }
                        ),
                        400,
                    )
                ...
                if (
                    ...
                    return make_response(
                        jsonify(
                            {
                                "status": "error",
                                "msg": "You do not have permission to promote a user to Administrator role.",
                            }
                        ),
                        400,
                    )
                ...
                if result["status"]:
                    ...
                    return make_response(
                        jsonify(
                            {"status": "ok", "msg": "Changed user role successfully."}
                        ),
                        200,
                    )
                else:
                    return make_response(
                        jsonify(
                            {
                                "status": "error",
                                "msg": "Cannot change user role. {0}".format(
                                    result["msg"]
                                ),
                            }
                        ),
...
@views_blueprint.route("/admin/account/edit/<account_name>", methods=["GET", "POST"])
@views_blueprint.route("/admin/account/edit", methods=["GET", "POST"])
@login_required
@operator_role_required
def admin_editaccount(account_name=None):
    ...
    if request.method == "POST":
        ...
        if result["status"]:
            ...
            history.add()
            return redirect(url_for("views_blueprint.admin_manageaccount"))
...
@views_blueprint.route("/admin/manageaccount", methods=["GET", "POST"])
@login_required
@operator_role_required
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if jdata["action"] == "delete_account":
                ...
                if result:
                    history = History(
                        msg="Delete account {0}".format(data),
                        created_by=current_user.username,
                    )
                    history.add()
                    return make_response(
                        jsonify({"status": "ok", "msg": "Account has been removed."}),
                        200,
                    )
                else:
                    return make_response(
                        jsonify({"status": "error", "msg": "Cannot remove account."}),
                        500,
                    )
            else:
                return make_response(
...
@views_blueprint.route("/admin/history", methods=["GET", "POST"])
@login_required
@operator_role_required
def admin_history():
    if request.method == "POST":
        ...
        if result:
            history = History(
                msg="Remove all histories", created_by=current_user.username
            )
            history.add()
            return make_response(
                jsonify({"status": "ok", "msg": "Changed user role successfully."}), 200
            )
        else:
            return make_response(
                jsonify({"status": "error", "msg": "Can not remove histories."}), 500
            )
...
@views_blueprint.route("/admin/setting/basic", methods=["GET"])
@login_required
@operator_role_required
...
@views_blueprint.route("/admin/setting/basic/<path:setting>/edit", methods=["POST"])
@login_required
@operator_role_required
def admin_setting_basic_edit(setting):
    ...
    if result:
        return make_response(
            jsonify({"status": "ok", "msg": "Toggled setting successfully."}), 200
        )
    else:
        return make_response(
            jsonify({"status": "error", "msg": "Unable to toggle setting."}), 500
        )


@views_blueprint.route("/admin/setting/basic/<path:setting>/toggle", methods=["POST"])
@login_required
@operator_role_required
...
@views_blueprint.route("/admin/setting/pdns", methods=["GET", "POST"])
@login_required
@admin_role_required
...
@views_blueprint.route("/admin/setting/dns-records", methods=["GET", "POST"])
@login_required
@operator_role_required
def admin_setting_records():
    if request.method == "GET":
        ...
        return render_template(
            "admin_setting_records.html", f_records=f_records, r_records=r_records
        )
    elif request.method == "POST":
        fr = {}
        rr = {}
        ...
        return redirect(url_for("views_blueprint.admin_setting_records"))


@views_blueprint.route("/admin/setting/authentication", methods=["GET", "POST"])
@login_required
@admin_role_required
    ...
    if request.method == "GET":
        ...
        if conf_type == "general":
            ...
            if not local_db_enabled and not Setting().get("ldap_enabled"):
                result = {
                    "status": False,
                    "msg": "Local DB and LDAP Authentication can not be disabled at the same time.",
                }
            else:
            ...
            if not ldap_enabled and not Setting().get("local_db_enabled"):
                ...
                Setting().set(
                    "ldap_admin_username", request.form.get("ldap_admin_username")
                )
                Setting().set(
                    "ldap_admin_password", request.form.get("ldap_admin_password")
                )
                Setting().set(
                    "ldap_filter_basic", request.form.get("ldap_filter_basic")
                )
                Setting().set(
                    "ldap_filter_username", request.form.get("ldap_filter_username")
                )
                Setting().set(
                    "ldap_sg_enabled",
                    True if request.form.get("ldap_sg_enabled") == "ON" else False,
                )
                Setting().set("ldap_admin_group", request.form.get("ldap_admin_group"))
                Setting().set(
                    "ldap_operator_group", request.form.get("ldap_operator_group")
                )
            ...
            Setting().set(
                "google_authorize_url", request.form.get("google_authorize_url")
            )
            ...
            Setting().set(
                "github_oauth_secret", request.form.get("github_oauth_secret")
            )
            Setting().set("github_oauth_scope", request.form.get("github_oauth_scope"))
            Setting().set(
                "github_oauth_api_url", request.form.get("github_oauth_api_url")
            )
            Setting().set(
                "github_oauth_token_url", request.form.get("github_oauth_token_url")
            )
            Setting().set(
            ...
            Setting().set(
                "oidc_oauth_token_url", request.form.get("oidc_oauth_token_url")
            )
            Setting().set(
                "oidc_oauth_authorize_url", request.form.get("oidc_oauth_authorize_url")
            )
            result = {
                "status": True,
                "msg": "Saved successfully. Please reload PDA to take effect.",
...
@views_blueprint.route("/user/profile", methods=["GET", "POST"])
@login_required
def user_profile():
    if request.method == "GET":
    ...
    if request.method == "POST":
        if session["authentication_type"] == "LOCAL":
            ...
            logging.warning(
                "Authenticated externally. User {0} information will not allowed to update the profile".format(
                ...
                if session["authentication_type"] in ["LOCAL", "LDAP"]:
                    ...
                    return make_response(
                        jsonify(
                            {
                                "status": "ok",
                                "msg": "Change OTP Authentication successfully. Status: {0}".format(
                                    enable_otp
                                ),
                            }
                        ),
                    ...
                    return make_response(
                        jsonify(
                            {
                                "status": "error",
                                "msg": "User {0} is externally. You are not allowed to update the OTP".format(
                                    current_user.username
                                ),
                            }
                        ),
            ...
            if session["authentication_type"] in ["LOCAL", "LDAP"]:
                ...
                if file:
                    ...
                    if file_extension.lower() in ["jpg", "jpeg", "png"]:
                        ...
                        file.save(
                            os.path.join(
                                app.config["UPLOAD_DIR"], "avatar", save_file_name
                            )
                        )
            else:
                logging.error(
                    "Authenticated externally. User {0} is not allowed to update the avatar"
                )
                abort(400)
...
@views_blueprint.route("/user/avatar/<path:filename>")
def user_avatar(filename):
    return send_from_directory(
        os.path.join(app.config["UPLOAD_DIR"], "avatar"), filename
    )


@views_blueprint.route("/qrcode")
@login_required
def qrcode():
    if not current_user:
        return redirect(url_for("views_blueprint.index"))
...
@views_blueprint.route("/nic/checkip.html", methods=["GET", "POST"])
@csrf.exempt
def dyndns_checkip():
    # route covers the default ddclient 'web' setting for the checkip service
    return render_template(
        "dyndns.html",
        response=request.environ.get("HTTP_X_REAL_IP", request.remote_addr),
    )


@views_blueprint.route("/nic/update", methods=["GET", "POST"])
...
def dyndns_update():
    ...
    if not domain:
        history = History(
            msg="DynDNS update: attempted update of {0} but it does not exist for this user".format(
                hostname
            ),
            created_by=current_user.username,
        )
    ...
    remote_addr = utils.validate_ipaddress(
        request.headers.get("X-Forwarded-For", request.remote_addr).split(", ")[:1]
    )
    ...
    for ip in myip_addr or remote_addr:
        ...
        if r.exists(domain.name) and r.is_allowed_edit():
            if r.data == str(ip):
                ...
                history = History(
                    msg="DynDNS update: attempted update of {0} but record did not change".format(
                        hostname
                    ),
                    created_by=current_user.username,
                )
                ...
                if result["status"] == "ok":
                    history = History(
                        msg="DynDNS update: updated {0} record {1} in zone {2}, it changed from {3} to {4}".format(
                            rtype, hostname, domain.name, oldip, str(ip)
                        ),
                        detail=str(result),
                        created_by=current_user.username,
                    )
            ...
            if (ondemand_creation is not None) and (
                strtobool(ondemand_creation.value) == True
            ):
                record = Record(
                    name=hostname, type=rtype, data=str(ip), status=False, ttl=3600
                )
                result = record.add(domain.name)
                if result["status"] == "ok":
                    history = History(
                        msg="DynDNS update: created record {0} in zone {1}, it now represents {2}".format(
                            hostname, domain.name, str(ip)
                        ),
                        detail=str(result),
                        created_by=current_user.username,
                    )
