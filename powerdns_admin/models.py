...
class User(db.Model):
    ...
    def __init__(
        ...
        if reload_info:
            user_info = (
                self.get_user_info_by_id() if id else self.get_user_info_by_username()
            )
    ...
    def ldap_search(self, searchFilter, baseDN):
        ...
        try:
            ...
            if Setting().get("ldap_type") == "ad":
                conn.simple_bind_s(
                    "{0}@{1}".format(self.username, Setting().get("ldap_domain")),
                    self.password,
                )
            else:
                conn.simple_bind_s(
        ...
        if method == "LOCAL":
            ...
            if user_info:
                if user_info.password and self.check_password(user_info.password):
                    logging.info(
                        'User "{0}" logged in successfully. Authentication request from {1}'.format(
                            self.username, src_ip
                        )
                    )
                    return True
                logging.error(
                    'User "{0}" inputted a wrong password. Authentication request from {1}'.format(
                        self.username, src_ip
                    )
                )
                return False
            ...
            logging.warning(
                'User "{0}" does not exist. Authentication request from {1}'.format(
                    self.username, src_ip
                )
            )
            return False
        ...
        if method == "LDAP":
            ...
            if Setting().get("ldap_type") == "ad":
                ldap_username = "{0}@{1}".format(
                    self.username, Setting().get("ldap_domain")
                )
                if not self.ldap_auth(ldap_username, self.password):
                    logging.error(
                        'User "{0}" input a wrong LDAP password. Authentication request from {1}'.format(
                            self.username, src_ip
                        )
                    )
                    return False
            ...
            if not ldap_result:
                logging.warning(
                    'LDAP User "{0}" does not exist. Authentication request from {1}'.format(
                        self.username, src_ip
                    )
                )
                return False
                        ...
                        try:
                            if LDAP_TYPE == "ldap":
                                if self.ldap_search(searchFilter, LDAP_ADMIN_GROUP):
                                    ...
                                    logging.info(
                                        'User {0} is part of the "{1}" group that allows admin access to PowerDNS-Admin'.format(
                                            self.username, LDAP_ADMIN_GROUP
                                        )
                                    )
                                elif self.ldap_search(
                                    searchFilter, LDAP_OPERATOR_GROUP
                                ):
                                    role_name = "Operator"
                                    logging.info(
                                        'User {0} is part of the "{1}" group that allows operator access to PowerDNS-Admin'.format(
                                            self.username, LDAP_OPERATOR_GROUP
                                        )
                                    )
                                elif self.ldap_search(searchFilter, LDAP_USER_GROUP):
                                    logging.info(
                                        'User {0} is part of the "{1}" group that allows user access to PowerDNS-Admin'.format(
                                            self.username, LDAP_USER_GROUP
                                        )
                                    )
                                else:
                                    logging.error(
                                        'User {0} is not part of the "{1}", "{2}" or "{3}" groups that allow access to PowerDNS-Admin'.format(
                                            self.username,
                                            LDAP_ADMIN_GROUP,
                                            LDAP_OPERATOR_GROUP,
                                            LDAP_USER_GROUP,
                                ...
                                if not user_ad_member_of:
                                    logging.error(
                                        "User {0} does not belong to any group while LDAP_GROUP_SECURITY_ENABLED is ON".format(
                                            self.username
                                        )
                                    )
                                    return False
                                ...
                                for group in [
                                    g.decode("utf-8") for g in user_ad_member_of
                                ]:
                            ...
                            logging.error(
                                'LDAP group lookup for user "{0}" has failed. Authentication request from {1}'.format(
                                    self.username, src_ip
                                )
                            )
                            logging.debug(traceback.format_exc())
            ...
            if not User.query.filter(User.username == self.username).first():
                ...
                try:
                    ...
                    if LDAP_TYPE == "ldap":
                        self.firstname = ldap_result[0][0][1]["givenName"][0].decode(
                            "utf-8"
                        )
    ...
    def get_apikeys(self, domain_name=None):
        ...
        apikey_query = (
            ...
            .filter(
                db.or_(DomainUser.user_id == User.id, AccountUser.user_id == User.id)
            )
    ...
    def create_local_user(self):
        ...
        self.password = (
            self.get_hashed_password(self.plain_text_password)
            if self.plain_text_password
            else "*"
        )
    ...
    def update_profile(self, enable_otp=None):
        ...
        user.password = (
            self.get_hashed_password(self.plain_text_password).decode("utf-8")
            if self.plain_text_password
            else user.password
        )
    ...
    def delete(self):
        ...
        try:
            ...
            db.session.rollback()
            logging.error(
                "Cannot delete user {0} from DB. DETAIL: {1}".format(self.username, e)
            )
            return False
...
class Account(db.Model):
    ...
    def grant_privileges(self, new_user_list):
        ...
        new_user_ids = (
            [u.id for u in User.query.filter(User.username.in_(new_user_list)).all()]
            if new_user_list
            else []
        )
        ...
        try:
            ...
            db.session.rollback()
            logging.error(
                "Cannot revoke user privileges on account {0}. DETAIL: {1}".format(
                    self.name, e
                )
            )
        ...
        try:
            ...
            db.session.rollback()
            logging.error(
                "Cannot grant user privileges to account {0}. DETAIL: {1}".format(
                    self.name, e
                )
            )
    ...
    def add_user(self, user):
        ...
        try:
            ...
            db.session.rollback()
            logging.error(
                "Cannot add user privileges on account {0}. DETAIL: {1}".format(
                    self.name, e
                )
            )
            return False
...
class Domain(db.Model):
    ...
    apikeys = db.relationship(
        "ApiKey", secondary=domain_apikey, back_populates="domains"
    )
    ...
    def get_domains(self):
        ...
        jdata = utils.fetch_json(
            urljoin(
                self.PDNS_STATS_URL, self.API_EXTENDED_URL + "/servers/localhost/zones"
            ),
            headers=headers,
        )
        return jdata
    ...
    def update_soa_setting(self, domain_name, soa_edit_api):
        ...
        try:
            ...
            logging.debug(traceback.format_exc())
            logging.error(
                "Cannot change soa-edit-api for domain {0}".format(domain_name)
            )
    ...
    def create_reverse_domain(self, domain_name, domain_reverse_name):
        ...
        domain_auto_ptr = (
            DomainSetting.query.filter(DomainSetting.domain == domain_obj)
        ...
        if None == domain_id and (system_auto_ptr or domain_auto_ptr):
            result = self.add(domain_reverse_name, "Master", "DEFAULT", "", "")
            self.update()
            if result["status"] == "ok":
                history = History(
                    msg="Add reverse lookup domain {0}".format(domain_reverse_name),
            ...
            if 0 != len(domain_users):
                ...
                return {
                    "status": "ok",
                    "msg": "New reverse lookup domain created with granted privileges",
                }
            return {
                "status": "ok",
    ...
    def get_user(self):
        ...
        user_ids = []
        query = (
            db.session.query(DomainUser, Domain)
    ...
    def grant_privileges(self, new_user_list):
        ...
        try:
            ...
            db.session.rollback()
            logging.error(
                "Cannot revoke user privileges on domain {0}. DETAIL: {1}".format(
                    self.name, e
                )
            )
        ...
        try:
            ...
            db.session.rollback()
            logging.error(
                "Cannot grant user privileges to domain {0}. DETAIL: {1}".format(
                    self.name, e
                )
            )
            ...
            try:
                jdata = utils.fetch_json(
                    urljoin(
                        self.PDNS_STATS_URL,
                        self.API_EXTENDED_URL
                        + "/servers/localhost/zones/{0}/cryptokeys".format(domain.name),
                    ),
                    headers=headers,
                    method="GET",
                )
        ...
        if domain:
            ...
            try:
                ...
                if "error" in jdata:
                    return {
                        "status": "error",
                        "msg": "API-RECTIFY could not be enabled for this domain",
                        "jdata": jdata,
                    }
                ...
                if "error" in jdata:
                    return {
                        "status": "error",
                        "msg": "Cannot enable DNSSEC for this domain. Error: {0}".format(
                            jdata["error"]
                        ),
                        "jdata": jdata,
                    }
        ...
        if domain:
            ...
            try:
                ...
                if jdata != True:
                    return {
                        "status": "error",
                        "msg": "Cannot disable DNSSEC for this domain. Error: {0}".format(
                            jdata["error"]
                        ),
                        "jdata": jdata,
                    }
                ...
                if "error" in jdata:
                    return {
                        "status": "error",
                        "msg": "API-RECTIFY could not be disabled for this domain",
                        "jdata": jdata,
                    }
...
class Record(object):
    ...
    def get_record_data(self, domain):
        ...
        try:
            ...
            logging.error(
                "Cannot fetch domain's record data from remote powerdns api. DETAIL: {0}".format(
                    e
                )
            )
            return False
    ...
    def add(self, domain):
        ...
        if check:
            ...
            if r["type"] in ("A", "AAAA", "CNAME"):
                return {
                    "status": "error",
                    "msg": 'Record already exists with type "A", "AAAA" or "CNAME"',
                }
                ...
                "rrsets": [
                    {
                        ...
                        "changetype": "REPLACE",
                        "ttl": self.ttl,
                        "records": [{"content": self.data, "disabled": self.status}],
                    }
                ]
            }
        else:
        ...
        try:
            ...
            logging.error(
                "Cannot add record {0}/{1}/{2} to domain {3}. DETAIL: {4}".format(
    ...
    def compare(self, domain_name, new_records):
        ...
        deleted_records = [
            ...
            if [x["name"], x["type"]] in list_deleted_records
            and (
    ...
    def apply(self, domain, post_records):
        ...
        for r in post_records:
            ...
            r_data = (
                domain
                if r_type == "CNAME" and r["record_data"] in ["@", ""]
                else r["record_data"]
            )
        ...
        final_records = []
        records = sorted(
            records, key=lambda item: (item["name"], item["type"], item["changetype"])
        )
        for key, group in itertools.groupby(
            records, lambda item: (item["name"], item["type"], item["changetype"])
        ):
            if self.NEW_SCHEMA:
                r_name = key[0]
                r_type = key[1]
                ...
                for item in group:
                    ...
                    new_record["records"].append(
                        {"content": temp_content, "disabled": temp_disabled}
                    )
        ...
        logging.info(
            urljoin(
                self.PDNS_STATS_URL,
                self.API_EXTENDED_URL + "/servers/localhost/zones/{0}".format(domain),
            )
        )
        try:
            headers = {}
            ...
            try:
                ...
                for r in new_records:
                    if r["type"] in ["A", "AAAA"]:
                        ...
                        self.name = (
                            dns.reversename.from_address(r_content)
                            .to_text()
                            .rstrip(".")
                        )
                        self.type = "PTR"
                ...
                logging.error(
                    "Cannot update auto-ptr record changes to domain {0}. DETAIL: {1}".format(
                        domain, e
                    )
                )
                return {
    ...
    def delete(self, domain):
        ...
        try:
            ...
            logging.error(
                "Cannot remove record {0}/{1}/{2} from domain {3}. DETAIL: {4}".format(
                    self.name, self.type, self.data, domain, e
                )
            )
            return {
                ...
                "rrsets": [
                    {
                        ...
                        "ttl": self.ttl,
                        "changetype": "REPLACE",
                        "records": [{"content": content, "disabled": self.status}],
                    }
                ]
            }
        else:
    ...
    def update_db_serial(self, domain):
        ...
        if domain:
            ...
            return {
                "status": False,
                "msg": "Could not find domain name {0} in local db".format(domain),
            }
...
class Setting(db.Model):
    ...
    def get_records_allow_to_edit(self):
        return list(
            set(
                self.get_forward_records_allow_to_edit()
...
class ApiKey(db.Model):
    ...
    domains = db.relationship(
        "Domain", secondary=domain_apikey, back_populates="apikeys"
    )
    ...
    def update(self, role_name=None, description=None, domains=None):
        try:
            ...
            if domains:
                domain_object_list = Domain.query.filter(Domain.name.in_(domains)).all()
    ...
    def get_hashed_password(self, plain_text_password=None):
        ...
        return bcrypt.hashpw(pw.encode("utf-8"), app.config.get("SALT").encode("utf-8"))
