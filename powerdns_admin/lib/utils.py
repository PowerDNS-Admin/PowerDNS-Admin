...
def fetch_json(remote_url, method="GET", data=None, params=None, headers=None):
    ...
    try:
        ...
        raise RuntimeError(
            "Error while loading JSON data from {0}".format(remote_url)
        ) from e
    return data
...
def display_time(amount, units="s", remove_seconds=True):
    ...
    amount = int(amount)
    INTERVALS = [
        (lambda mlsec: divmod(mlsec, 1000), "ms"),
    ...
    if app.config["SAML_ENABLED"]:
        ...
        if "SAML_IDP_ENTITY_ID" in app.config:
            idp_data = OneLogin_Saml2_IdPMetadataParser.parse_remote(
                ...
                entity_id=app.config.get("SAML_IDP_ENTITY_ID", None),
                required_sso_binding=app.config["SAML_IDP_SSO_BINDING"],
            )
        else:
            idp_data = OneLogin_Saml2_IdPMetadataParser.parse_remote(
                app.config["SAML_METADATA_URL"],
