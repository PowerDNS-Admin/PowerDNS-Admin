...
from powerdns_admin.errors import RequestIsNotJSON, NotEnoughPrivileges
from powerdns_admin.errors import DomainAccessForbidden
...
def can_configure_dnssec(f):
    ...
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if g.user.role.name not in ["Administrator", "Operator"] and Setting().get(
            "dnssec_admins_only"
        ):
            return redirect(url_for("error", code=401))
...
def can_create_domain(f):
    ...
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if g.user.role.name not in ["Administrator", "Operator"] and not Setting().get(
...
def api_basic_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if auth_header:
            ...
            user = User(
                username=username, password=password, plain_text_password=password
            )
        ...
        if auth_header:
            ...
            try:
                ...
                auth = apikey.is_validate(
                    method=auth_method, src_ip=request.remote_addr
                )
