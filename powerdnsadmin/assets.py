...
class RemoveLegacyCssPropertiesFilter(Filter):
    """Remove obsolete vendor declarations rejected by current browsers."""

    def input(self, _in, out, **kwargs):
        css = _in.read()
        css = re.sub(
            r'(^|[;{])\s*-moz-border-radius\s*:[^;{}]+;',
            r'\1',
            css,
            flags=re.MULTILINE,
        )
...
css_login = Bundle(
    ...
    filters=(RemoveLegacyCssPropertiesFilter, 'rcssmin', 'cssrewrite'),
