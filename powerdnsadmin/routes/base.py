...
from flask import render_template, url_for, redirect, session, request, current_app, jsonify
from flask_login import LoginManager
from flask_session import Session  # Import Flask-Session
from flask_wtf.csrf import CSRFProtect, CSRFError  # Import CSRFProtect
...
def login_via_authorization_header_or_remote_user(request):
    ...
    if remote_user and current_app.config.get('REMOTE_USER_ENABLED'):
        ...
        auth_method = 'LOCAL' if session_remote_user else current_app.config.get(
            'REMOTE_AUTH_METHOD', 'LDAP')
        ...
        valid_remote_user = load_if_valid(
            user, method=auth_method, src_ip=request.remote_addr, trust_user=True)
